...
const Agent360Dashboard = () => {
  ...
  const getBranchRankings = (getAllData = false) => {
    const currentMonthData = [
      { agency: '글로벌금융판매', branch: '글로벌화이브스타', achievement: 115.2, ape: 145, isActive: true },
      { agency: '글로벌금융판매', branch: '하나돔', achievement: 112.8, ape: 138, isActive: true },
      { agency: '글로벌금융판매', branch: '리더스에프엔', achievement: 108.5, ape: 132, isActive: true },
      { agency: '지금용코리아', branch: '서울', achievement: 105.7, ape: 128, isActive: true },
      { agency: '메타리치', branch: '보험스토어', achievement: 103.2, ape: 125, isActive: true },
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg shadow-sm border p-6">
              ...
              <div className="text-center mb-6">
                <div className="text-4xl font-black text-blue-700 mb-2">{myKPI.goalAchievement.current.toFixed(1)}%</div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-4 mt-2 relative">
                  <div className="bg-blue-500 h-4 rounded-full transition-all" style={{width: `${myKPI.goalAchievement.current}%`}}></div>
                  {/* 영업본부 평균 노란선 */}
                  <div 
                    className="absolute top-0 h-4 w-0.5 bg-yellow-500 z-10"
                    style={{left: `${myKPI.goalAchievement.hqAvg}%`}}
                  />
                  {/* 본부 평균 퍼센트 표시 */}
                  <div 
                    className="absolute -bottom-5 text-xs text-yellow-600"
                    style={{left: `${myKPI.goalAchievement.hqAvg}%`, transform: 'translateX(-50%)'}}
                  >
                    강남본부 {myKPI.goalAchievement.hqAvg}%
                  </div>
                </div>
              ...
              <div className="space-y-3 mt-2">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">전월 동기 대비</span>
                  <span className="font-medium text-green-600">▲ {myKPI.goalAchievement.vsLastMonth}%p</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">남은 금액</span>
                  <span className="font-medium text-gray-900">{formatCurrency(myKPI.goalAchievement.gap * 10000)}</span>
                </div>
                <div className="bg-blue-50 rounded-lg p-3 text-center">
                  <div className="text-sm text-gray-700">이번달 목표 달성을 위해 하루 평균 <span className="font-semibold text-blue-600">{formatCurrency(myKPI.goalAchievement.dailyRequired * 10000)}</span>이 필요해요!</div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white rounded-lg shadow-sm border p-4">
                ...
                <div className="text-center mb-3">
                  <div className="text-2xl font-bold text-blue-600">{myKPI.designerActivity.current}%</div>
              ...
              <div className="bg-white rounded-lg shadow-sm border p-4">
                ...
                <div className="text-center mb-3">
                  <div className="text-2xl font-bold text-blue-600">{myKPI.mobileContract.current}%</div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setShowAllBranchesModal(false)}>
          <div className="bg-white rounded-lg p-6 max-w-6xl w-full mx-4 max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto">
              {/* 테이블 헤더 */}
              <div className="bg-gray-100 rounded-t-lg border-b sticky top-0">
                {/* 메인 헤더 */}
              ...
              <div className="space-y-0">
                {(() => {
                  const sortedData = getBranchRankings(true).data.map(branch => ({
                    ...branch,
                    threeMonthAvg: branch.ape === 0 ? 0 : Math.round((branch.ape + (branch.previousApe || Math.round(branch.ape * 0.85)) + Math.round(branch.ape * 0.9)) / 3),
