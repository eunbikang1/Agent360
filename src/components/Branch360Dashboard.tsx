          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg shadow-sm border p-6">
              ...
              <div className="text-center mb-6">
                <div className="text-4xl font-black text-blue-700 mb-2">{corePerformance.achievementRate.toFixed(1)}%</div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-4 mt-2 relative">
                  <div className="bg-blue-500 h-4 rounded-full transition-all" style={{width: `${corePerformance.achievementRate}%`}}></div>
                  {/* 기대 진도선 (15일/22일 = 68.2%) */}
                  <div
                    className="absolute top-0 h-4 w-0.5 bg-orange-500 z-10"
                    style={{left: `${(15/22)*100}%`}}
                  />
                  {/* 기대 진도 퍼센트 표시 */}
                  <div
                    className="absolute -bottom-5 text-xs text-orange-600"
                    style={{left: `${(15/22)*100}%`, transform: 'translateX(-50%)'}}
                  >
                    기대 진도 {Math.round((15/22)*100)}%
                  </div>
                </div>

                {/* 하루 평균 필요 금액 안내 */}
                <div className="bg-gray-50 rounded-lg p-3 mt-6">
                  <div className="text-sm text-gray-700">이번달 목표 달성을 위해 하루 평균 <span className="font-semibold text-blue-600">{formatCurrency((corePerformance.targetApe - corePerformance.currentApe) / 7 * 10000)}</span>이 필요해요!</div>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3 mt-4 pt-3 border-t">
                <div className="text-center">
                  <div className="text-xs text-gray-500">전월 동기 대비</div>
                  <div className={`font-semibold ${corePerformance.achievementVsLastMonth > 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {corePerformance.achievementVsLastMonth > 0 ? '▲' : '▼'} {Math.abs(corePerformance.achievementVsLastMonth)}%p
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-xs text-gray-500">남은 금액</div>
                  <div className="font-semibold text-blue-600">{formatCurrency(Math.abs(corePerformance.targetApe - corePerformance.currentApe) * 10000)}</div>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t">
                <div className="bg-blue-50 rounded-lg p-4">
                  <div className="text-sm font-semibold text-gray-700 mb-3 text-center">기여도</div>
                  <div className="text-xs text-gray-600 text-center mb-3">
                    해당 지점은 <strong>{corePerformance.managerName}</strong> 지점장님 APE 목표의 <strong>{corePerformance.managerPlanContribution}%</strong> 담당,<br/>
                    현재까지 전체 실적의 <strong>{corePerformance.managerContributionRate}%</strong> 기여
                  </div>
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div className="text-center">
                      <div className="text-sm font-medium text-gray-900">
                        {formatCurrency(corePerformance.branchTargetApe * 10000)}
                        <span className="text-gray-500">/</span>
                        {formatCurrency(corePerformance.managerPersonalTarget * 10000)}
                      </div>
                      <div className="text-gray-500">지점 목표/개인 목표</div>
                    </div>
                    <div className="text-center">
                      <div className="text-sm font-medium text-gray-900">
                        {formatCurrency(corePerformance.currentApe * 10000)}
                        <span className="text-gray-500">/</span>
                        {formatCurrency(corePerformance.managerApe * 10000)}
                      </div>
                      <div className="text-gray-500">지점 실적/개인 실적</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div className="text-center p-3 bg-blue-50 rounded-lg group relative">
                    <div className="text-lg font-bold text-blue-600">170건</div>
                    <div className="text-xs text-gray-600">월 청약</div>
                  ...
                  <div className="text-center p-3 bg-green-50 rounded-lg group relative">
                    <div className="text-lg font-bold text-green-600">4.5만원</div>
                    <div className="text-xs text-gray-600">월납보험료</div>
